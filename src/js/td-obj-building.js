...
_TD.a.push(function (TD) {
  ...
  var building_obj = {
    ...
    updateBtnDesc: function () {
      this.scene.panel.btn_upgrade.desc = TD._t("upgrade", [
        TD._t("building_name_" + this.type),
        this.level + 1,
        this.getUpgradeCost()
      ]);

      this.scene.panel.btn_sell.desc = TD._t("sell", [
        TD._t("building_name_" + this.type),
        this.getSellMoney()
      ]);
    },
   ...
   // TD.log("remove building #" + this.id + ".");
    ...
    findTaget: function () {
      ...
      var cx = this.cx, cy = this.cy, range2 = Math.pow(this.range_px, 2);

      // 如果当前建筑有目标，并且目标还是有效的，并且目标仍在射程内
      if (this.target && this.target.is_valid && Math.pow(this.target.cx - cx, 2) + Math.pow(this.target.cy - cy, 2) <= range2) {
        return;
      }

      // 在进入射程的怪物中寻找新的目标
      this.target = TD.lang.any(TD.lang.rndSort(this.map.monsters), /* 将怪物随机排序 */
        function (obj) {
    ...
    _upgrade2: function (k) {
      ...
      var mk = "max_" + k;
      var uk = "_upgrade_rule_" + k;
      var uf = this[uk] || TD.default_upgrade_rule;
    ...
    upgrade: function () {
      ...
      /* 可升级的变量 */
      var attrs = ["damage", "range", "speed", "life", "shield"];
  ...
  var bullet_obj = {
    ...
    checkHit: function () {
      ...
      var r = this.r * _TD.retina;

      var monster = this.map.anyMonster(function (obj) {
