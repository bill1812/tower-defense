...
_TD.a.push(function (TD) {
  ...
  var monster_obj = {
    _init: function (cfg) {
      ...
      if (this.speed > cfg.max_speed) {
        this.speed = cfg.max_speed;
      }
      this.life = this.life0 = Math.floor(attr.life * (this.difficulty + 1) * (Math.random() + 0.5) * 0.5);
      ...
      if (this.r < (4 * _TD.retina)) {
        this.r = 4 * _TD.retina;
      }

      if (this.r > TD.grid_size / 2 - (4 * _TD.retina)) {
        this.r = TD.grid_size / 2 - (4 * _TD.retina);
      }
   ...
   // delete fw;
    ...
    getNextGrid: function () {
      if (this.way.length == 0 || Math.random() < 0.1 /* 有 1/10 的概率自动重新寻路 */) {
        this.findWay();
      }
    ...
    chkIfBlocked: function (mx, my) {
      ...
      var fw = new TD.FindWay(
        ...
        this.map.exit.mx, this.map.exit.my,
        function (x, y) {
          return !(x == mx && y == my) && _this.map.checkPassable(x, y);
        }
      );
    ...
    step: function () {
      ...
      if (this.cx == this.next_grid.cx && this.cy == this.next_grid.cy) {
        ...
        var dpy = this.next_grid.cy - this.cy;
        var sx = dpx < 0 ? -1 : 1;
        var sy = dpy < 0 ? -1 : 1;
    ...
    onEnter: function () {
      var msg, balloontip = this.scene.panel.balloontip;

      if (balloontip.el == this) {
        balloontip.hide();
        balloontip.el = null;
      } else {
        msg = TD._t("monster_info", [this.life, this.shield, this.speed, this.damage]);
